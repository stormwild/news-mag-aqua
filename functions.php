...
function news_mag_aqua_setup() {
	 ...
	 * to change 'news-mag-aqua' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'news-mag-aqua' );
    ...
    */
	...
	register_nav_menus( array(
		'top-menu' => __( 'Top Menu', 'news-mag-aqua' ), //main menu at top.
	) );
...
add_action( 'after_setup_theme', 'news_mag_aqua_setup' );
 ...
 */
function news_mag_aqua_scripts() {
	global $wp;

	// Add google fonts
	if ( ign_get_config( 'google_fonts' ) ) {
		wp_enqueue_style( 'news-mag-aqua-fonts', ign_google_fonts_url(), array(), wp_get_theme()->get( 'Version' ) );
	}


	// Theme stylesheet. Will get this stylesheet or a child themes stylesheet.
	wp_enqueue_style( 'news-mag-aqua-style', get_stylesheet_uri(), '', wp_get_theme()->get( 'Version' ) );

	//Sass compiles styles. Will get child's theme version if found instead. Child theme should import with sass.
	wp_enqueue_style( 'news-mag-aqua-sass-styles', get_theme_file_uri( '/dist/frontEnd.css' ), '', wp_get_theme()->get( 'Version' ) );
	...
	wp_enqueue_script( 'news-mag-aqua-custom-js', get_template_directory_uri() . '/dist/frontEnd_bundle.js', array(
		'jquery',
		'polyfill'
	),
	...
	wp_localize_script( 'news-mag-aqua-custom-js', 'frontEndAjax', array(
	...
	wp_localize_script( 'news-mag-aqua-custom-js', 'icons', array(
...
add_action( 'wp_enqueue_scripts', 'news_mag_aqua_scripts' );
...
function ign_gutenberg_styles() {
	...
	wp_enqueue_script( 'news-mag-aqua-custom-js', get_template_directory_uri() . '/dist/custom' . $suffix . '.js', array( 'jquery' ),
		wp_get_theme()->get( 'Version' ), true );
}
...
function footer_styles() {
	...
	//$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
	wp_enqueue_script( 'iconify', 'https://code.iconify.design/1/1.0.6/iconify.min.js' );
	wp_enqueue_style( 'news-mag-aqua-admin-styles', get_theme_file_uri( '/dist/backEnd.css' ), '', wp_get_theme()->get( 'Version' ) );
}
...
if ( ! function_exists( 'ign_widgets_init' ) ) {
	function ign_widgets_init() {
		register_sidebar( array(
			'name'          => __( 'Sidebar', 'news-mag-aqua' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here to appear in your sidebar.', 'news-mag-aqua' ),
...
//footer widgets and sections. up to 4
		register_sidebar( array(
			'name'          => esc_html__( 'Footer', 'news-mag-aqua' ),
			'id'            => 'sidebar-2',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 2', 'news-mag-aqua' ),
			'id'            => 'sidebar-3',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 3', 'news-mag-aqua' ),
			'id'            => 'sidebar-4',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 4', 'news-mag-aqua' ),
			'id'            => 'sidebar-5',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
 ...
 */
function news_mag_aqua_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
	}
}

add_action( 'wp_head', 'news_mag_aqua_pingback_header' );
